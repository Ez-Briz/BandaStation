...
#define INIT_ORDER_OUTDOOR_EFFECTS	100
...
/proc/station_time(display_only = FALSE, wtime=world.time)
	return (((wtime - SSticker.round_start_time) + SSticker.round_start_time) % 864000)
	// - (display_only? GLOB.timezoneOffset : 0)
...
/proc/daytimeDiff(timeA, timeB)
	...
	return time_diff // normalise with the time rate multiplier
...
/datum/controller/subsystem/outdoor_effects/proc/check_cycle()
	...
	if(station_time() + skip_timer > next_step_datum.start)
		if(next_day)
			return FALSE
		get_time_of_day()
		return TRUE
